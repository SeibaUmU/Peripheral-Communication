...
void interrupt()
{
  if(PIR2.USBIF == 1)
  {
    ...
    USB_Interrupt_Proc();       // Xu ly ngat USB, xem them trong file USBcontrol.h
  }

  if(INTCON.INT0IF == 1)
  {
    INTCON.INT0IF = 0;          // Clear interrupt bit
    count++; // Tang bien dem so lan nhan SW0
    writebuff[0] = count; // Luu so lan nhan SW0 vao byte 0 cua bo dem ghi
    oldstate = 1; // Co trang thai thay doi, cho phep gui du lieu ve HOST trong vong lap main
...
void main(void)
{
  ...
  // Cau hinh Port B
  PORTB = 0x00; LATB = 0x00; // Khoi tao gia tri cho Port B
  ...
  INTCON2.INTEDG0 = 1;   // Ngat ngoai RB0 ngat khi co su thay doi tu 1 sang 0
  INTCON2.INTEDG1 = 1;
  INTCON2.INTEDG2 = 1;
