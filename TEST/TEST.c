...
void interrupt(void){
    ...
    if (INTCON3.INT1IF) { //ngat ngoai tai RB1
     ...
     if(PIR1.RCIF==1) // ngat uart1 khi co data nhan duoc
     {
      ...
      {//led re0
       if (strncmp(strstr(buf_string_receive,"le_on"),"le_on",len_code) == 0) //so sanh chuoi lenh nhan duoc voi ma lenh
       {
        LATE.LATE0 = led_on; // bat led re0
        UART1_Write_Text("@Lle0_on&"); //gui phan hoi, voi du lieu dc gui co dang @...&
       }
       else if (strncmp(strstr(buf_string_receive,"le_of"),"le_of",len_code) == 0)
       {
